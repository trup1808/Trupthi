...
public class SpecialDemo {
		...
		private Special[] sI = new Special[110010];
		...
		public void add(){
		 ...
		 try{			
					...
					RandomAccessFile indexfile = new RandomAccessFile ("secondary5.txt","rw");
					indexfile.seek(indexfile.length());
					indexfile.writeBytes(specialization+","+pos+"\n");
					indexfile.close();
				}
				catch(IOException e){
			...
			public void secIndex(){
				...
				try {
					indexfile = new RandomAccessFile("secondary5.txt","rw");
					...
					try {
						...
						while((line = indexfile.readLine())!= null){
							...
							sI[sIIndex] = new Special();
							sI[sIIndex].setRecPos(pos);
							sI[sIIndex].setSpecial(sec);
							reccount++;
							sIIndex++;
		                                        if(sIIndex==110010)
		                                        {
			...
			public void sortIndex() {
				...
				for(int i=0; i<reccount; i++)
				    {	
						for(int j=i+1; j<reccount; j++) {
							if(sI[i].getSpecial().compareTo(sI[j].getSpecial())  > 0)
				            {
				                temp.setSpecial(sI[i].getSpecial()); 
						        temp.setRecPos(sI[i].getRecPos());
						
					        	sI[i].setSpecial(sI[j].getSpecial());
					        	sI[i].setRecPos(sI[j].getRecPos());
						
					        	sI[j].setSpecial(temp.getSpecial());
				...
				public void search(){
		             ...
		             do {
		                 ...
		                 pos--;
		                 if (pos < 0) { break;}
		             }while(sI[pos].getSpecial().compareTo(specialization)==0);
		             
		             pos = oripos + 1 ;
		          // if (pos > reccount-1) { return;}
		             
		             while(sI[pos].getSpecial().compareTo(specialization)==0 && pos > reccount-1) {
		                 readFile(pos);
		                 pos++;
		 ...
		 public void readFile(int pos) {
			 ...
			 if (pos == -1) {
		        ...
		        int binarySearch(Special s[], int l, int r, String specialization) {
		    	
		    	int mid;
		    	while (l<=r) {
		            
		    		mid = (l+r)/2;
		    		if(s[mid].getSpecial().compareTo(specialization)==0)
		    		{
		    			return mid;
		    			}
		    		if(s[mid].getSpecial().compareTo(specialization)<0)
		...
		public   void delete() throws IOException {
		     ...
		     System.out.println("Enter the specialization to delete: ");
		     Scanner scanner = new Scanner(System.in);
		     String specialization = scanner.next();
		     String ans = "n";
		     int pos;
		 ...
		 @SuppressWarnings("resource")
		public void markDelete(String specialization,int pos) {
		     try {
		         RandomAccessFile recordfilee = new RandomAccessFile ("C:\\Users\\Admin\\Desktop\\ohh\\t.csv","rw");
		         RandomAccessFile indexfilee = new RandomAccessFile ("secondary5.txt","rw");
		                 ...
		                 while(st.hasMoreTokens()) {
		                  ...
		                  //   System.out.println("indexPos: " + indexPos);
		                    // System.out.println("getrecpos: " + Long.parseLong(sI[pos].getRecPos()));
		                     if(indexName.equals(specialization) && indexPos == Long.parseLong(sI[pos].getRecPos()) ) {
